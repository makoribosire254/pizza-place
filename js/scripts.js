...
$ (document).ready(function() {
	...
	$("#calcBtn").click(function() {	
		...
		if (errorMsg==null || errorMsg=="") {
			...
			var sizeName = $('input[name="selectedSize"]:checked + label').text();		
			var sizePrice = arraySize[size.value].price;
			console.log(sizeName + " Ksh" + sizePrice);

			var crustName = $('input[name="selectedSize"]:checked + label').text();	
			var crustPrice = arrayCrust[crust.value].price;
			console.log(crustName + " Ksh" + crustPrice);

			var sauceName = $('input[name="selectedSize"]:checked + label').text();		
			var saucePrice = 0;
			console.log(sauceName + " Ksh" + saucePrice);

			var cheeseName = $('input[name="selectedSize"]:checked + label').text();		
			var cheesePrice = arrayCheese[cheese.value].price;
			console.log(cheeseName + " Ksh" + cheesePrice);

			// -- from checkbox groups
			var meatName = meat.join(", ");
			var meatPrice = meat.length - 1;
			if (meatPrice < 0) { meatPrice=0 };
			console.log(meatName + " Ksh" + meatPrice);

			var veggieName = veggie.join(", ");
			var veggiePrice = veggie.length - 1;
			if (veggiePrice < 0) { veggiePrice=0 };
			console.log(veggieName + " Ksh" + veggiePrice);
			...
			itemPrices[0].innerHTML = "Ksh" + sizePrice.toFixed(2);

			itemNames[1].innerHTML = crustName;
			if (crustPrice > 0) {itemPrices[1].innerHTML = "Ksh" + crustPrice.toFixed(2)};

			itemNames[2].innerHTML = sauceName;


			itemNames[3].innerHTML = cheeseName;
			if (cheesePrice > 0) {itemPrices[3].innerHTML = "Ksh" + cheesePrice.toFixed(2)};

			itemNames[4].innerHTML = meatName;
			if (meatPrice > 0) {itemPrices[4].innerHTML = "Ksh" + meatPrice.toFixed(2)};

			itemNames[5].innerHTML = veggieName;
			if (veggiePrice > 0) itemPrices[5].innerHTML = "Ksh" + veggiePrice.toFixed(2);

			itemTotal.innerHTML = "Ksh" + total.toFixed(2);
